...
query_router = Agent(
    ...
    backstory=(
        ...
        "- **infosec**: Information Security policies - passwords, access control, data protection\n"
        ...
        "- **procurement_standards**: Abu Dhabi Procurement Standards - government regulations\n"
        "- **procurement_all**: Ambiguous procurement queries (searches all procurement docs)\n"
        "- **all**: Non-specific queries (searches everything)\n\n"
...
insight_synthesizer = Agent(
    ...
    backstory=(
        ...
        "CHUNK FORMAT:\n"
        "Each chunk you receive is formatted as:\n"
        "---\n"
        "**Source:** filename.pdf\n"
        "**Page:** page_number (if available)\n"
        "**Section Context:** brief context (if available)\n"
        "actual content here...\n"
        "---\n\n"

        "CRITICAL INSTRUCTIONS:\n"
        "- IGNORE any JSON routing decisions (e.g., {\"route\": \"...\"})\n"
        "- Use the actual document content to formulate your answer\n"
        "- Extract source names and page numbers from the **Source:** and **Page:** lines\n\n"

        "CORE PRINCIPLES:\n"
        "- Answer questions directly using information from the chunks\n"
        "- Use ONLY the provided context - never add outside knowledge\n"
        "- Be concise for simple questions, detailed for complex ones\n\n"

        "RESPONSE STYLE:\n"
        "- Start with the most direct answer to the question\n"
        "- Use bullet points, numbering, or paragraphs as needed\n"
        "- Make citations natural: 'According to the policy...' or 'As stated in Article X...'\n\n"

        "CITATION REQUIREMENTS:\n"
        "- ALWAYS include a **Sources:** section at the end\n"
        "- Use the EXACT filename from the **Source:** line\n"
        "- Include the page number from the **Page:** line\n"
        "- Reference Articles/Sections mentioned in the content\n"
        "- Format: **Sources:**\n"
        "  - HR Bylaws.pdf (Article 71, Page 62)\n"
        "  - Procurement Manual (Business Process).PDF (Section 2.6.2, Page 57)\n\n"

        "REMEMBER: Synthesize the chunks into a helpful answer with proper source citations!"
    ),
    llm=llm,
    verbose=True,
    allow_delegation=False,
